...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        ...
        if variable in heroku_var:
            await zed.edit("**⌔∮ تم تغيـر** `{}` **:**\n **- المتغير :** `{}` \n**- يتم الان اعـادة تشغيـل بـوت@R2RR7 يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(variable, value))
        else:
...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "جلب":
        ...
        await zed.edit(f"**⌔∮** `{variable}`  **تم حذفه بنجاح. \n**يتم الان اعـادة تشغيـل بـوت@R2RR7 يستغـرق الامر 2-1 دقيقـه ▬▭ ...**")
        del heroku_var[variable]
